...
- **状态管理**：当前仍以 React 本地状态 + `useMemo`/`useState` 为主，首页轮询逻辑直接写在 `App.tsx` 中；下一阶段计划引入 TanStack Query 管理请求缓存、轮询与错误处理。
- **图表库**：Lightweight Charts（K 线，已使用）+ Recharts（面积/柱状，已使用）。
- **HTTP 客户端**：已统一使用 Axios（封装于 `frontend/src/services/dashboard.ts`），集中配置 `baseURL`、超时与通用响应封装，后续可在拦截器中追加鉴权/错误提示。
- **路由**：已接入 React Router v7（首页 + 早报/日/周/月/年报路由），后续会结合权限与埋点进一步拆分模块。
