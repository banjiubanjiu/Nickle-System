...
# Human-friendly Chinese labels used when printing to CLI.
FIELD_LABELS = {
    "date": "日期",
    "contract": "合约",
    "latest_price": "最新价",
...
def _print_record(label: str, data: Optional[Dict[str, Optional[Any]]], field_order) -> None:
    ...
    for field in field_order:
        label_cn = FIELD_LABELS.get(field)
        display = f"{field} ({label_cn})" if label_cn else field
        print(f"{display:>28}: {data.get(field)}")
